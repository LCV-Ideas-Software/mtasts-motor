...
const APPROVED_ZIZMOR_RELEASES = new Map([
  ["v2.3.0", "67ecbebba92a6973f889114d4b0d64596519a94f"],
]);
...
function assertZizmorReleaseReference(workflow) {
  ...
  assert.equal(
    APPROVED_ZIZMOR_RELEASES.get(releaseRefs[0][2]),
    releaseRefs[0][1],
    "the Zizmor reference must match an approved signed release",
  );
  assert.equal(
...
test("the Zizmor boundary rejects a different SHA for an approved release", () => {
  const differentSha = zizmorWorkflow.replace(
    /@[0-9a-f]{40} # zizmor\/v2\.3\.0/,
    `@${"0".repeat(40)} # zizmor/v2.3.0`,
  );
  assert.throws(
    () => assertZizmorReleaseReference(differentSha),
    /approved signed release/,
  );
});

test("the Zizmor boundary rejects a different release for an approved SHA", () => {
  const differentRelease = zizmorWorkflow.replace(
    "# zizmor/v2.3.0",
    "# zizmor/v2.3.1",
  );
  assert.throws(
    () => assertZizmorReleaseReference(differentRelease),
    /approved signed release/,
  );
});
